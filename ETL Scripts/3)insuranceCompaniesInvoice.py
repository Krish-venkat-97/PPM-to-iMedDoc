...
src_insurance_companies0  = 'SELECT DISTINCT InsuranceCompany FROM InvoiceHeadSummary'
src_insurance_companies_df0 = pd.read_sql(src_insurance_companies0, get_src_accessdb_connection())

src_insurance_companies1  = 'SELECT DISTINCT InsuranceCompany1 FROM InvoiceHeadSummary'
...
src_insurance_companies_df = src_insurance_companies_df.drop_duplicates(subset=['InsuranceCompany']).reset_index(drop=True)
...
bar = tqdm(total=len(src_insurance_companies_dfx),desc='Inserting Insurance Companies from InvoiceHeadSummary',position=0)
...
for index,row in src_insurance_companies_dfx.iterrows():
    ...
    try:
        ...
        VALUES ({safe_value(row['InsuranceCompany'])}, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0, NULL, 1, 1, 0, CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), NULL, 'From_InvoiceHeadSummary');
        """
