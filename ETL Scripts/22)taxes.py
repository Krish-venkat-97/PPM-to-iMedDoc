...
# Convert VATPercent to string for comparison
src_tax_df['VATPercent'] = src_tax_df['VATPercent'].astype(str)
existing_tax_ids = pd.read_sql("SELECT perc FROM taxes", myconnection)
existing_tax_ids = existing_tax_ids['perc'].astype(str).tolist()
